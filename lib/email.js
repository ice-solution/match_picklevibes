const fs = require("fs");
const path = require("path");
const nodemailer = require("nodemailer");
const { divisionLabelOrValue, divisionFeeCents } = require("./divisions");

function getMailerConfig() {
  return {
    host: process.env.EMAIL_HOST,
    port: parseInt(process.env.EMAIL_PORT || "587", 10),
    secure: String(process.env.EMAIL_SECURE || "").trim() === "true", // Gmail STARTTLS 通常為 false
    auth: {
      user: process.env.EMAIL_USER,
      pass: process.env.EMAIL_PASS
    }
  };
}

function transporter() {
  const cfg = getMailerConfig();
  if (!cfg.host || !cfg.port || !cfg.auth.user || !cfg.auth.pass) return null;
  return nodemailer.createTransport(cfg);
}

function fmtDateISO(d) {
  try {
    return new Date(d).toISOString().slice(0, 10);
  } catch (e) {
    return "—";
  }
}

function genderText(g) {
  if (g === "male") return "男";
  if (g === "female") return "女";
  return "—";
}

function duprText(n) {
  if (n == null || Number.isNaN(Number(n))) return "—";
  return Number(n).toFixed(2);
}

function duprOrNR(player) {
  if (player && player.kidNoDuprScore === true) return "沒有 DUPR 積分";
  if (player && player.duprNR === true) return "NR";
  return duprText(player && player.dupr);
}

function escapeHtml(s) {
  return String(s || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/\"/g, "&quot;")
    .replace(/'/g, "&#039;");
}

function referrerPhoneDisplay(reg) {
  const s = reg && reg.referrerPhone != null ? String(reg.referrerPhone).trim() : "";
  return s || "—";
}

function buildRegistrationEmail({ tournament, reg }) {
  const subject = `【登記完成】${tournament.name}｜稍後通知抽籤結果及付款連結`;

  const summaryLines = [
    `比賽：${tournament.name}`,
    `日期：${tournament.date}`,
    `地點：${tournament.location}`,
    "",
    `組別：${divisionLabelOrValue(reg.division)}`,
    "",
    `聯絡人：${reg.fullName}`,
    `電郵：${reg.email}`,
    `電話：${reg.phone}`,
    `推薦人電話：${referrerPhoneDisplay(reg)}`,
    `推廣碼：${reg.bocReferralCode || "—"}`,
    "",
    "球員 1：",
    `- 姓名：${reg.player1?.name || "—"}`,
    `- 出生日期：${reg.player1?.dateOfBirth ? fmtDateISO(reg.player1.dateOfBirth) : "—"}`,
    `- 性別：${genderText(reg.player1?.gender)}`,
    `- DUPR：${duprOrNR(reg.player1)}`,
    "",
    "球員 2：",
    `- 姓名：${reg.player2?.name || "—"}`,
    `- 出生日期：${reg.player2?.dateOfBirth ? fmtDateISO(reg.player2.dateOfBirth) : "—"}`,
    `- 性別：${genderText(reg.player2?.gender)}`,
    `- DUPR：${duprOrNR(reg.player2)}`,
    "",
    reg.notes ? `備註：\n${reg.notes}` : null,
    "",
    "我們會於稍後通知抽籤結果及付款連結，請留意電郵。"
  ].filter(Boolean);

  const text = summaryLines.join("\n");

  const html = `
  <div style="font-family: Arial, 'Noto Sans TC', sans-serif; background:#f6f8fc; padding:24px;">
    <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid rgba(0,0,0,.06);">
      <div style="background:linear-gradient(135deg,#09B7BC,#F7C300,#E6027E);padding:18px 20px;color:#fff;">
        <div style="font-size:14px;font-weight:700;letter-spacing:.5px;">登記完成</div>
        <div style="font-size:20px;font-weight:800;margin-top:6px;">${escapeHtml(tournament.name)}</div>
        <div style="opacity:.95;margin-top:6px;font-size:13px;">稍後通知抽籤結果及付款連結</div>
      </div>

      <div style="padding:18px 20px; color:#0B1220;">
        <p style="margin:0 0 12px 0; color:rgba(0,0,0,.7);">
          你好，以下為你的登記資料摘要。請保留此電郵作紀錄。
        </p>

        <div style="background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:8px;">賽事資訊</div>
          <div style="font-size:13px;line-height:1.7;">
            <div><b>日期：</b>${escapeHtml(tournament.date)}</div>
            <div><b>地點：</b>${escapeHtml(tournament.location)}</div>
            <div><b>組別：</b>${escapeHtml(divisionLabelOrValue(reg.division))}</div>
          </div>
        </div>

        <div style="margin-top:14px; background:#fff;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:8px;">聯絡人資料</div>
          <div style="font-size:13px;line-height:1.7;">
            <div><b>姓名：</b>${escapeHtml(reg.fullName)}</div>
            <div><b>電郵：</b>${escapeHtml(reg.email)}</div>
            <div><b>電話：</b>${escapeHtml(reg.phone)}</div>
            <div><b>推薦人電話：</b>${escapeHtml(referrerPhoneDisplay(reg))}</div>
            <div><b>推廣碼：</b>${escapeHtml(reg.bocReferralCode || "—")}</div>
          </div>
        </div>

        <div style="margin-top:14px; display:grid; grid-template-columns: 1fr; gap:10px;">
          <div style="background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
            <div style="font-weight:800;margin-bottom:8px;">球員 1</div>
            <div style="font-size:13px;line-height:1.7;">
              <div><b>姓名：</b>${escapeHtml(reg.player1?.name || "—")}</div>
              <div><b>出生日期：</b>${reg.player1?.dateOfBirth ? escapeHtml(fmtDateISO(reg.player1.dateOfBirth)) : "—"}</div>
              <div><b>性別：</b>${escapeHtml(genderText(reg.player1?.gender))}</div>
              <div><b>DUPR：</b>${escapeHtml(duprOrNR(reg.player1))}</div>
            </div>
          </div>

          <div style="background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
            <div style="font-weight:800;margin-bottom:8px;">球員 2</div>
            <div style="font-size:13px;line-height:1.7;">
              <div><b>姓名：</b>${escapeHtml(reg.player2?.name || "—")}</div>
              <div><b>出生日期：</b>${reg.player2?.dateOfBirth ? escapeHtml(fmtDateISO(reg.player2.dateOfBirth)) : "—"}</div>
              <div><b>性別：</b>${escapeHtml(genderText(reg.player2?.gender))}</div>
              <div><b>DUPR：</b>${escapeHtml(duprOrNR(reg.player2))}</div>
            </div>
          </div>
        </div>

        ${reg.notes ? `
          <div style="margin-top:14px; background:#fff;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
            <div style="font-weight:800;margin-bottom:8px;">備註</div>
            <div style="white-space:pre-wrap; font-size:13px;line-height:1.7; color:rgba(0,0,0,.75);">${escapeHtml(reg.notes)}</div>
          </div>
        ` : ""}

        <div style="margin-top:14px; padding:14px 14px; border-radius:12px; background:rgba(9,183,188,.10); border:1px solid rgba(9,183,188,.20);">
          <div style="font-weight:800;">下一步</div>
          <div style="margin-top:6px; font-size:13px; line-height:1.7; color:rgba(0,0,0,.75);">
            我們會於稍後通知抽籤結果及付款連結，請留意電郵。
          </div>
        </div>
      </div>

      <div style="padding:14px 20px; font-size:12px; color:rgba(0,0,0,.55); border-top:1px solid rgba(0,0,0,.06);">
        此為系統自動發送電郵，請勿直接回覆。
      </div>
    </div>
  </div>
  `.trim();

  return { subject, text, html };
}

async function sendRegistrationEmail({ tournament, reg }) {
  const t = transporter();
  if (!t) {
    return { ok: false, error: "Email transporter not configured" };
  }
  const fromName = process.env.EMAIL_FROM_NAME || "PickleVibes";
  const from = process.env.EMAIL_FROM || `${fromName} <${process.env.EMAIL_USER}>`;
  const { subject, text, html } = buildRegistrationEmail({ tournament, reg });
  await t.sendMail({
    from,
    to: reg.email,
    subject,
    text,
    html
  });
  return { ok: true };
}

function emailBannerAttachments() {
  const bannerPath = path.join(__dirname, "..", "public", "images", "email_banner.png");
  try {
    if (fs.existsSync(bannerPath)) {
      return [{ filename: "email_banner.png", path: bannerPath, cid: "pv_email_banner" }];
    }
  } catch (_e) {
    /* ignore */
  }
  return [];
}

function formatHkdFromCents(cents) {
  const n = Number(cents);
  if (!Number.isFinite(n)) return "—";
  return `HK$${(n / 100).toFixed(2)}`;
}

function buildPaymentRequestEmail({ tournament, reg, checkoutUrl, amountCents, publicBaseUrl }) {
  const paymentWindowLine =
    String(process.env.PAYMENT_EMAIL_DEADLINE_LINE || "").trim() ||
    "2026年5月4日 00:00 至 2026年5月11日 23:59（香港時間 GMT+8）";
  const paymentForfeitLine =
    String(process.env.PAYMENT_EMAIL_FORFEIT_LINE || "").trim() ||
    "如於上述限期內仍未完成付款，將視為未完成報名付款程序；如需繼續參賽請按主辦方後續指示辦理。";

  const cents =
    amountCents != null && Number.isFinite(Number(amountCents))
      ? Number(amountCents)
      : divisionFeeCents(reg.division);
  const amountText = formatHkdFromCents(cents);
  const bannerAttachments = emailBannerAttachments();
  const fallbackBannerUrl = `${String(publicBaseUrl || "")
    .trim()
    .replace(/\/$/, "")}/images/email_banner.png`;
  const bannerSrcAttr =
    bannerAttachments.length > 0 ? "cid:pv_email_banner" : escapeHtml(fallbackBannerUrl);

  const subject = `【付款通知】${tournament.name}｜請於限期內完成報名費付款（${amountText}）`;

  const summaryLines = [
    `你好 ${reg.fullName || ""}，`,
    "",
    "感謝你參加本次賽事報名。請仔細核對以下資料是否正確，並請於電郵內連結前往 Stripe 安全付款頁完成報名費付款。",
    "",
    `比賽：${tournament.name}`,
    `日期：${tournament.date}`,
    `地點：${tournament.location}`,
    "",
    `組別：${divisionLabelOrValue(reg.division)}`,
    `報名費：${amountText}`,
    "",
    `付款限期：${paymentWindowLine}`,
    paymentForfeitLine,
    "",
    `聯絡人：${reg.fullName}`,
    `電郵：${reg.email}`,
    `電話：${reg.phone}`,
    `推薦人電話：${referrerPhoneDisplay(reg)}`,
    `推廣碼：${reg.bocReferralCode || "—"}`,
    "",
    "球員 1：",
    `- 姓名：${reg.player1?.name || "—"}`,
    `- 出生日期：${reg.player1?.dateOfBirth ? fmtDateISO(reg.player1.dateOfBirth) : "—"}`,
    `- 性別：${genderText(reg.player1?.gender)}`,
    `- DUPR：${duprOrNR(reg.player1)}`,
    "",
    "球員 2：",
    `- 姓名：${reg.player2?.name || "—"}`,
    `- 出生日期：${reg.player2?.dateOfBirth ? fmtDateISO(reg.player2.dateOfBirth) : "—"}`,
    `- 性別：${genderText(reg.player2?.gender)}`,
    `- DUPR：${duprOrNR(reg.player2)}`,
    "",
    reg.notes ? `備註：\n${reg.notes}` : null,
    "",
    `付款連結：\n${checkoutUrl}`,
    "",
    "付款完成後，系統會用電郵／網站流程確認（請保留 Stripe 收據）。如有問題請聯絡主辦方。"
  ].filter(Boolean);

  const text = summaryLines.join("\n");

  const html = `
  <div style="font-family: Arial, 'Noto Sans TC', sans-serif; background:#f6f8fc; padding:24px;">
    <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid rgba(0,0,0,.06);">
      <div style="line-height:0;">
        <img src="${bannerSrcAttr}" alt="" width="640" style="width:100%;max-width:640px;height:auto;display:block;border:0;outline:none;" />
      </div>

      <div style="padding:18px 20px; color:#0B1220;">
        <div style="font-size:18px;font-weight:900;">感謝你參加</div>
        <div style="margin-top:6px;font-size:14px;line-height:1.7;color:rgba(0,0,0,.72);">
          請核對以下報名資料，並點擊下方按鈕前往 <b>Stripe</b> 付款頁完成報名費付款。
        </div>

        <div style="margin-top:14px;text-align:center;">
          <a href="${escapeHtml(checkoutUrl)}" style="display:inline-block;background:#635BFF;color:#fff;text-decoration:none;font-weight:900;padding:12px 18px;border-radius:12px;">
            前往付款頁（${escapeHtml(amountText)}）
          </a>
          <div style="margin-top:10px;font-size:12px;line-height:1.7;color:rgba(0,0,0,.55);">
            若按鈕無法開啟，請複製此
            <a href="${escapeHtml(checkoutUrl)}" style="font-weight:900; color:#635BFF; text-decoration:underline;">
              連結
            </a>
            到瀏覽器。
          </div>
        </div>

        <div style="margin-top:14px; background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:8px;">賽事資訊</div>
          <div style="font-size:13px;line-height:1.7;">
            <div><b>日期：</b>${escapeHtml(tournament.date)}</div>
            <div><b>地點：</b>${escapeHtml(tournament.location)}</div>
            <div><b>組別：</b>${escapeHtml(divisionLabelOrValue(reg.division))}</div>
            <div><b>報名費：</b>${escapeHtml(amountText)}</div>
          </div>
        </div>

        <div style="margin-top:14px; background:#fff;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:8px;">聯絡人資料</div>
          <div style="font-size:13px;line-height:1.7;">
            <div><b>姓名：</b>${escapeHtml(reg.fullName)}</div>
            <div><b>電郵：</b>${escapeHtml(reg.email)}</div>
            <div><b>電話：</b>${escapeHtml(reg.phone)}</div>
            <div><b>推薦人電話：</b>${escapeHtml(referrerPhoneDisplay(reg))}</div>
            <div><b>推廣碼：</b>${escapeHtml(reg.bocReferralCode || "—")}</div>
          </div>
        </div>

        <div style="margin-top:14px; display:grid; grid-template-columns: 1fr; gap:10px;">
          <div style="background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
            <div style="font-weight:800;margin-bottom:8px;">球員 1</div>
            <div style="font-size:13px;line-height:1.7;">
              <div><b>姓名：</b>${escapeHtml(reg.player1?.name || "—")}</div>
              <div><b>出生日期：</b>${reg.player1?.dateOfBirth ? escapeHtml(fmtDateISO(reg.player1.dateOfBirth)) : "—"}</div>
              <div><b>性別：</b>${escapeHtml(genderText(reg.player1?.gender))}</div>
              <div><b>DUPR：</b>${escapeHtml(duprOrNR(reg.player1))}</div>
            </div>
          </div>

          <div style="background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
            <div style="font-weight:800;margin-bottom:8px;">球員 2</div>
            <div style="font-size:13px;line-height:1.7;">
              <div><b>姓名：</b>${escapeHtml(reg.player2?.name || "—")}</div>
              <div><b>出生日期：</b>${reg.player2?.dateOfBirth ? escapeHtml(fmtDateISO(reg.player2.dateOfBirth)) : "—"}</div>
              <div><b>性別：</b>${escapeHtml(genderText(reg.player2?.gender))}</div>
              <div><b>DUPR：</b>${escapeHtml(duprOrNR(reg.player2))}</div>
            </div>
          </div>
        </div>

        ${reg.notes ? `
          <div style="margin-top:14px; background:#fff;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
            <div style="font-weight:800;margin-bottom:8px;">備註</div>
            <div style="white-space:pre-wrap; font-size:13px;line-height:1.7; color:rgba(0,0,0,.75);">${escapeHtml(reg.notes)}</div>
          </div>
        ` : ""}

        <div style="margin-top:14px; padding:14px 14px; border-radius:12px; background:rgba(230,2,126,.07); border:1px solid rgba(230,2,126,.22);">
          <div style="font-weight:800; color:#0B1220;">付款限期及重要須知</div>
          <div style="margin-top:8px; font-size:13px; line-height:1.75; color:rgba(0,0,0,.78);">
            <div><b>付款時間：</b>${escapeHtml(paymentWindowLine)}</div>
            <div style="margin-top:10px;">${escapeHtml(paymentForfeitLine)}</div>
            <div style="margin-top:10px;">請使用此電郵提供的 Stripe 連結於限期內完成付款；連結由 Stripe 托管，安全可靠。</div>
          </div>
        </div>
      </div>

      <div style="padding:14px 20px; font-size:12px; color:rgba(0,0,0,.55); border-top:1px solid rgba(0,0,0,.06);">
        此為系統自動發送電郵，請勿直接回覆。
      </div>
    </div>
  </div>
  `.trim();

  return { subject, text, html, attachments: bannerAttachments };
}

async function sendPaymentRequestEmail({ tournament, reg, checkoutUrl, amountCents, publicBaseUrl }) {
  const t = transporter();
  if (!t) {
    return { ok: false, error: "Email transporter not configured" };
  }
  const fromName = process.env.EMAIL_FROM_NAME || "PickleVibes";
  const from = process.env.EMAIL_FROM || `${fromName} <${process.env.EMAIL_USER}>`;
  const { subject, text, html, attachments } = buildPaymentRequestEmail({
    tournament,
    reg,
    checkoutUrl,
    amountCents,
    publicBaseUrl
  });
  await t.sendMail({
    from,
    to: reg.email,
    subject,
    text,
    html,
    attachments
  });
  return { ok: true };
}

function buildPaymentReminderEmail({ tournament, reg, payLinkUrl, amountCents, publicBaseUrl }) {
  const paymentWindowLine =
    String(process.env.PAYMENT_EMAIL_DEADLINE_LINE || "").trim() ||
    "2026年5月4日 00:00 至 2026年5月11日 23:59（香港時間 GMT+8）";
  const paymentForfeitLine =
    String(process.env.PAYMENT_EMAIL_FORFEIT_LINE || "").trim() ||
    "如於上述限期內仍未完成付款，將視為未完成報名付款程序；如需繼續參賽請按主辦方後續指示辦理。";

  const cents =
    amountCents != null && Number.isFinite(Number(amountCents))
      ? Number(amountCents)
      : divisionFeeCents(reg.division);
  const amountText = formatHkdFromCents(cents);

  const bannerAttachments = emailBannerAttachments();
  const fallbackBannerUrl = `${String(publicBaseUrl || "")
    .trim()
    .replace(/\/$/, "")}/images/email_banner.png`;
  const bannerSrcAttr =
    bannerAttachments.length > 0 ? "cid:pv_email_banner" : escapeHtml(fallbackBannerUrl);

  const subject = `【付款提醒】${tournament.name}｜請於限期內完成報名費付款（${amountText}）`;

  const summaryLines = [
    `你好 ${reg.fullName || ""}，`,
    "",
    `溫馨提醒：你已成功登記「${tournament.name}」，但目前仍未完成付款。請於限期內完成付款，以完成報名手續。`,
    "",
    `付款時間：${paymentWindowLine}`,
    paymentForfeitLine,
    "",
    `付款連結：\n${payLinkUrl}`,
    "",
    `組別：${divisionLabelOrValue(reg.division)}`,
    `報名費：${amountText}`,
    "",
    `聯絡電話：${reg.phone || "—"}`,
    `推薦人電話：${referrerPhoneDisplay(reg)}`,
    "",
    "如有問題請聯絡主辦方，謝謝。"
  ].filter(Boolean);

  const text = summaryLines.join("\n");

  const html = `
  <div style="font-family: Arial, 'Noto Sans TC', sans-serif; background:#f6f8fc; padding:24px;">
    <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid rgba(0,0,0,.06);">
      <div style="line-height:0;">
        <img src="${bannerSrcAttr}" alt="" width="640" style="width:100%;max-width:640px;height:auto;display:block;border:0;outline:none;" />
      </div>

      <div style="padding:18px 20px; color:#0B1220;">
        <div style="font-size:18px;font-weight:900;">付款提醒</div>
        <div style="margin-top:6px;font-size:14px;line-height:1.7;color:rgba(0,0,0,.72);">
          你好 ${escapeHtml(reg.fullName || "")}，溫馨提醒你目前仍未完成付款。請於限期內完成付款以完成報名手續。
        </div>

        <div style="margin-top:14px; padding:14px 14px; border-radius:12px; background:rgba(230,2,126,.07); border:1px solid rgba(230,2,126,.22);">
          <div style="font-weight:800; color:#0B1220;">付款限期及重要須知</div>
          <div style="margin-top:8px; font-size:13px; line-height:1.75; color:rgba(0,0,0,.78);">
            <div><b>付款時間：</b>${escapeHtml(paymentWindowLine)}</div>
            <div style="margin-top:10px;">${escapeHtml(paymentForfeitLine)}</div>
          </div>
        </div>

        <div style="margin-top:14px;text-align:center;">
          <a href="${escapeHtml(payLinkUrl)}" style="display:inline-block;background:#635BFF;color:#fff;text-decoration:none;font-weight:900;padding:12px 18px;border-radius:12px;">
            前往付款頁（${escapeHtml(amountText)}）
          </a>
          <div style="margin-top:10px;font-size:12px;line-height:1.7;color:rgba(0,0,0,.55);">
            若按鈕無法開啟，請按此
            <a href="${escapeHtml(payLinkUrl)}" style="font-weight:900; color:#635BFF; text-decoration:underline;">連結</a>
            到瀏覽器。
          </div>
        </div>

        <div style="margin-top:14px; background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:8px;">報名資料摘要</div>
          <div style="font-size:13px;line-height:1.7;">
            <div><b>組別：</b>${escapeHtml(divisionLabelOrValue(reg.division))}</div>
            <div><b>報名費：</b>${escapeHtml(amountText)}</div>
            <div style="margin-top:8px;"><b>聯絡電話：</b>${escapeHtml(reg.phone || "—")}</div>
            <div><b>推薦人電話：</b>${escapeHtml(referrerPhoneDisplay(reg))}</div>
            <div style="margin-top:8px;"><b>球員 1：</b>${escapeHtml(reg.player1?.name || "—")}｜DUPR：${escapeHtml(duprOrNR(reg.player1))}</div>
            <div><b>球員 2：</b>${escapeHtml(reg.player2?.name || "—")}｜DUPR：${escapeHtml(duprOrNR(reg.player2))}</div>
          </div>
        </div>
      </div>

      <div style="padding:14px 20px; font-size:12px; color:rgba(0,0,0,.55); border-top:1px solid rgba(0,0,0,.06);">
        此為系統自動發送電郵，請勿直接回覆。
      </div>
    </div>
  </div>
  `.trim();

  return { subject, text, html, attachments: bannerAttachments };
}

function fmtDateTimeHK(d) {
  try {
    return new Date(d).toLocaleString("zh-HK", { hour12: false, timeZone: "Asia/Hong_Kong" });
  } catch (e) {
    return "—";
  }
}

function buildPaymentInvoiceEmail({
  tournament,
  reg,
  amountCents,
  currency,
  stripeCheckoutSessionId,
  stripePaymentIntentId,
  paidAt,
  publicBaseUrl
}) {
  const cents =
    amountCents != null && Number.isFinite(Number(amountCents))
      ? Number(amountCents)
      : divisionFeeCents(reg.division);
  const amountText = formatHkdFromCents(cents);
  const cur = String(currency || "hkd").toUpperCase();
  const paidAtStr = paidAt ? fmtDateTimeHK(paidAt) : fmtDateTimeHK(new Date());
  const sessionId = String(stripeCheckoutSessionId || "").trim();
  const piId = String(stripePaymentIntentId || "").trim();
  const receiptRef = sessionId ? sessionId.slice(-12) : "—";

  const bannerAttachments = emailBannerAttachments();
  const fallbackBannerUrl = `${String(publicBaseUrl || "")
    .trim()
    .replace(/\/$/, "")}/images/email_banner.png`;
  const bannerSrcAttr =
    bannerAttachments.length > 0 ? "cid:pv_email_banner" : escapeHtml(fallbackBannerUrl);

  const subject = `【付款收據】${tournament.name}｜已收取報名費 ${amountText}`;

  const summaryLines = [
    `你好 ${reg.fullName || ""}，`,
    "",
    `感謝你的付款。以下為「${tournament.name}」報名費之入帳證明，請保留此電郵作紀錄。`,
    "",
    `收款金額：${amountText}（${cur}）`,
    `入帳時間（香港時間）：${paidAtStr}`,
    `組別：${divisionLabelOrValue(reg.division)}`,
    "",
    `Stripe Checkout Session：${sessionId || "—"}`,
    `Stripe PaymentIntent：${piId || "—"}`,
    "",
    `聯絡人：${reg.fullName || "—"}`,
    `電郵：${reg.email || "—"}`,
    `電話：${reg.phone || "—"}`,
    `推薦人電話：${referrerPhoneDisplay(reg)}`,
    "",
    "球員 1：" + (reg.player1?.name || "—") + "｜球員 2：" + (reg.player2?.name || "—"),
    "",
    "此收據由 Stripe 完成扣款後由系統自動發出；正式發票（如有）請另依主辦方安排。"
  ];

  const text = summaryLines.join("\n");

  const html = `
  <div style="font-family: Arial, 'Noto Sans TC', sans-serif; background:#f6f8fc; padding:24px;">
    <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid rgba(0,0,0,.06);">
      <div style="line-height:0;">
        <img src="${bannerSrcAttr}" alt="" width="640" style="width:100%;max-width:640px;height:auto;display:block;border:0;outline:none;" />
      </div>

      <div style="background:linear-gradient(135deg,#09B7BC,#F7C300,#E6027E);padding:18px 20px;color:#fff;">
        <div style="font-size:14px;font-weight:700;letter-spacing:.5px;">付款收據</div>
        <div style="font-size:20px;font-weight:800;margin-top:6px;">${escapeHtml(tournament.name)}</div>
        <div style="opacity:.95;margin-top:6px;font-size:13px;">已透過 Stripe 成功收取報名費</div>
      </div>

      <div style="padding:18px 20px; color:#0B1220;">
        <p style="margin:0 0 14px 0; color:rgba(0,0,0,.72); line-height:1.7;">
          你好 <b>${escapeHtml(reg.fullName || "")}</b>，感謝你的付款。以下為入帳證明，請保留此電郵作紀錄。
        </p>

        <div style="background:rgba(9,183,188,.10); border:1px solid rgba(9,183,188,.22); border-radius:14px; padding:16px 16px; text-align:center;">
          <div style="font-size:12px;font-weight:800;color:rgba(11,18,32,.55); letter-spacing:.06em;">已收金額</div>
          <div style="margin-top:6px;font-size:28px;font-weight:900;color:#0B1220;">${escapeHtml(amountText)}</div>
          <div style="margin-top:4px;font-size:12px;color:rgba(11,18,32,.55);">${escapeHtml(cur)} · 入帳時間（香港） ${escapeHtml(paidAtStr)}</div>
        </div>

        <div style="margin-top:14px; background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:8px;">交易參考</div>
          <div style="font-size:13px;line-height:1.75;">
            <div><b>收據編號（Session 尾碼）：</b>${escapeHtml(receiptRef)}</div>
            <div><b>Checkout Session ID：</b><span style="word-break:break-all;">${escapeHtml(sessionId || "—")}</span></div>
            <div><b>PaymentIntent ID：</b><span style="word-break:break-all;">${escapeHtml(piId || "—")}</span></div>
          </div>
        </div>

        <div style="margin-top:14px; background:#fff;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:8px;">賽事及組別</div>
          <div style="font-size:13px;line-height:1.7;">
            <div><b>日期：</b>${escapeHtml(tournament.date)}</div>
            <div><b>地點：</b>${escapeHtml(tournament.location)}</div>
            <div><b>組別：</b>${escapeHtml(divisionLabelOrValue(reg.division))}</div>
          </div>
        </div>

        <div style="margin-top:14px; background:#f6f8fc;border:1px solid rgba(0,0,0,.06);border-radius:12px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:8px;">報名聯絡人</div>
          <div style="font-size:13px;line-height:1.7;">
            <div><b>姓名：</b>${escapeHtml(reg.fullName || "—")}</div>
            <div><b>電郵：</b>${escapeHtml(reg.email || "—")}</div>
            <div><b>電話：</b>${escapeHtml(reg.phone || "—")}</div>
            <div><b>推薦人電話：</b>${escapeHtml(referrerPhoneDisplay(reg))}</div>
          </div>
        </div>

        <div style="margin-top:14px; padding:14px 14px; border-radius:12px; background:rgba(0,0,0,.03); border:1px solid rgba(0,0,0,.06); font-size:12px; line-height:1.65; color:rgba(11,18,32,.65);">
          此電郵於 Stripe 確認付款成功後由系統自動發出，作為入帳證明。如需正式發票（Invoice）或公司抬頭，請依主辦方公佈之方式聯絡辦理。
        </div>
      </div>

      <div style="padding:14px 20px; font-size:12px; color:rgba(0,0,0,.55); border-top:1px solid rgba(0,0,0,.06);">
        此為系統自動發送電郵，請勿直接回覆。
      </div>
    </div>
  </div>
  `.trim();

  return { subject, text, html, attachments: bannerAttachments };
}

async function sendPaymentInvoiceEmail({
  tournament,
  reg,
  amountCents,
  currency,
  stripeCheckoutSessionId,
  stripePaymentIntentId,
  paidAt,
  publicBaseUrl
}) {
  const t = transporter();
  if (!t) {
    return { ok: false, error: "Email transporter not configured" };
  }
  const fromName = process.env.EMAIL_FROM_NAME || "PickleVibes";
  const from = process.env.EMAIL_FROM || `${fromName} <${process.env.EMAIL_USER}>`;
  try {
    const { subject, text, html, attachments } = buildPaymentInvoiceEmail({
      tournament,
      reg,
      amountCents,
      currency,
      stripeCheckoutSessionId,
      stripePaymentIntentId,
      paidAt,
      publicBaseUrl
    });
    await t.sendMail({
      from,
      to: reg.email,
      subject,
      text,
      html,
      attachments
    });
    return { ok: true };
  } catch (err) {
    return { ok: false, error: String(err && err.message ? err.message : err || "send failed") };
  }
}

async function sendPaymentReminderEmail({ tournament, reg, payLinkUrl, amountCents, publicBaseUrl }) {
  const t = transporter();
  if (!t) {
    return { ok: false, error: "Email transporter not configured" };
  }
  const fromName = process.env.EMAIL_FROM_NAME || "PickleVibes";
  const from = process.env.EMAIL_FROM || `${fromName} <${process.env.EMAIL_USER}>`;
  const { subject, text, html, attachments } = buildPaymentReminderEmail({
    tournament,
    reg,
    payLinkUrl,
    amountCents,
    publicBaseUrl
  });
  await t.sendMail({
    from,
    to: reg.email,
    subject,
    text,
    html,
    attachments
  });
  return { ok: true };
}

module.exports = {
  duprOrNR,
  sendRegistrationEmail,
  buildRegistrationEmail,
  sendPaymentRequestEmail,
  buildPaymentRequestEmail,
  sendPaymentReminderEmail,
  buildPaymentReminderEmail,
  sendPaymentInvoiceEmail,
  buildPaymentInvoiceEmail
};

