...
/** 男單（單打）：一位球員 */
function isSinglesDivision(division) {
  const rule = RULES[String(division || "").trim()];
  return !!(rule && rule.pair === "M_SINGLE");
}
 ...
 * @param {{ dateOfBirth: Date, gender: string, dupr: number } | null} p2 - 單打組別可為 null
...
function validateDivisionPlayers(division, p1, p2, refDate) {
  ...
  if (rule.pair !== "M_SINGLE" && (!p1 || !p2)) {
    errors.push("組別資料不完整，請重新提交");
    return { ok: false, errors };
  }
  ...
  if (rule.pair === "M_SINGLE") {
    const msErrors = [];
    if (!p1) {
      msErrors.push("組別資料不完整，請重新提交");
      return { ok: false, errors: msErrors };
    }
    if (kidDiv) {
      msErrors.push("無法識別所選組別，請重新選擇");
      return { ok: false, errors: msErrors };
    }
    if (p1kidSkip) {
      msErrors.push("「沒有 DUPR 積分」只適用於小朋友組別");
      return { ok: false, errors: msErrors };
    }
    if (p2kidSkip || p2nr) {
      msErrors.push("單打組別只需填寫一位球員資料");
      return { ok: false, errors: msErrors };
    }

    const nrAllowedMs = allowNRForDivision(division);
    if (p1nr && !nrAllowedMs) {
      msErrors.push("此組別不接受 NR，請填寫 DUPR");
      return { ok: false, errors: msErrors };
    }

    const raw1 = p1nr ? null : (p1.duprRaw != null ? p1.duprRaw : p1.dupr);
    const dup1 = p1nr ? { ok: true, value: null } : parseDuprInput(raw1);
    if (!dup1.ok) msErrors.push(`球員 1：${dup1.error}`);
    if (msErrors.length) return { ok: false, errors: msErrors };

    const d1 = dup1.value;
    if (rule.dupr === "3.0-") {
      if (d1 != null && !(d1 < 3.0)) {
        msErrors.push(`球員 1：此組別要求 DUPR 3.0 以下（< 3.00），目前為 ${d1.toFixed(2)}`);
      }
    } else if (rule.dupr === "3.0+") {
      if (d1 == null) msErrors.push("球員 1：此組別不接受 NR，請填寫 DUPR");
      else if (!(d1 >= 3.0)) {
        msErrors.push(`球員 1：此組別要求 DUPR 3.0 或以上（≥ 3.00），目前為 ${d1.toFixed(2)}`);
      }
    }

    const a1 = ageOnDate(p1.dateOfBirth, refDate);
    if (rule.age === "30+") {
      if (a1 < 30) msErrors.push(`球員 1：依出生日期計算為 ${a1} 歲，未達 30+ 組別要求（需年滿 30 歲）`);
    } else if (rule.age === "50+") {
      if (a1 < 50) msErrors.push(`球員 1：依出生日期計算為 ${a1} 歲，未達 50+ 組別要求（需年滿 50 歲）`);
    }

    if (p1.gender !== GENDER.MALE) msErrors.push("男單組別：球員須為男性");

    return { ok: msErrors.length === 0, errors: msErrors };
  }
 ...
 * pair: MM | FF | MF | OPEN_M | OPEN_F | OPEN_MF | ANY | M_SINGLE
 */
const RULES = {
  ...
  "22 - 30+ - 3.0- - 男單": { dupr: "3.0-", age: "30+", pair: "M_SINGLE" },
  ...
  "21 - 公開 - - 男單公開": { dupr: "any", age: "open", pair: "M_SINGLE" },
