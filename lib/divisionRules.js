...
function parseDuprInput(raw) {
  ...
  if (rounded < 1.0 || rounded > 4.0) {
    return { ok: false, error: "DUPR 必須介乎 1.00 至 4.00" };
  }
...
function validateDivisionPlayers(division, p1, p2, refDate) {
  ...
  const raw1 = p1.duprRaw != null ? p1.duprRaw : p1.dupr;
  const raw2 = p2.duprRaw != null ? p2.duprRaw : p2.dupr;
  const dup1 = parseDuprInput(raw1);
  const dup2 = parseDuprInput(raw2);
  ...
  for (const [label, d] of [
    ...
    if (rule.dupr === "3.0-") {
      ...
      if (!(d >= 3.0)) errors.push(`${label}：此組別要求 DUPR 3.0 或以上（≥ 3.00），目前為 ${d.toFixed(2)}`);
    }
  }
...
/**
 * dupr: '3.0-' | '3.0+' | 'any'
 * age: '30+' | '50+' | 'open' | 8 | 10 | 12 | 14 (U8 = 未滿 8 即 age < 8)
